...
class QueueMonitorComponent extends BaseComponent {
    ...
    private bindEvents(): void {
        ...
        this.$el.on('input.aaxisQueueMon', '[data-role="filter"]', (e: any) => {
        ...
        $(document).on('mousedown.aaxisQueueMonPalette', (e: any) => {
    ...
    private onToggleAutoRefresh(event: any): void {
    ...
    private colorFor(name: string, index: number): string {
        if (!this.colors[name]) {
    ...
    private loadQueues(silent: boolean, selectDefault: boolean): void {
        $.ajax({url: this.queuesUrl, method: 'GET'})
            .done((response: {queues?: QueueSummary[]}) => {
            ...
            .fail((jqXhr: any) => {
                if (!silent) {
                    ...
                    this.$el.find('[data-role="queues"]').empty().append(
                        $('<li/>', {'class': 'aaxis-queue-mon__empty'}).text(message)
                    );
                }
            });
    }
    ...
    private renderQueues(): void {
        ...
        const visible = this.queues.filter(queue =>
            this.filterText === '' || queue.name.toLowerCase().indexOf(this.filterText) !== -1
        );
    ...
    private buildPalette(name: string): any {
        const $palette = $('<div/>', {
    ...
    private badge(kind: string, value: number, label: string): any {
        return $('<span/>', {
            'class': 'aaxis-queue-mon__badge is-' + kind + (value > 0 ? ' has-value' : ''),
            title: label
        }).append(
            $('<span/>', {'class': 'aaxis-queue-mon__badge-label', text: label}),
            $('<span/>', {'class': 'aaxis-queue-mon__badge-value', text: String(value)})
        );
    }

    // --- Selection -----------------------------------------------------------

    private onQueueToggle(event: any): void {
    ...
    private onSelectNone(event: any): void {
        event.preventDefault();
        this.selected.clear();
        this.details = {};
        this.renderQueues();
    ...
    private onColorSwatch(event: any): void {
        event.preventDefault();
    ...
    private onPaletteColor(event: any): void {
        event.preventDefault();
        event.stopPropagation();
    ...
    private orderedSelected(): string[] {
     ...
     */
    private samplesDisabled(): boolean {
        const loaded = this.orderedSelected()
            .map(name => this.details[name])
            .filter((detail): detail is QueueDetail => !!detail);
    ...
    private renderDetailStructure(): void {
    ...
    private seriesPoints(detail: QueueDetail | undefined): Array<{timestamp: number; value: number}> {
        if (!detail) {
            return [];
        }
    ...
    private renderChart(): void {
        ...
        const y = (v: number) => height - padBottom - (v / vMax) * (height - padTop - padBottom);
    ...
    private renderSamplesTable(): void {
        ...
        order.forEach(name => {
            const color = this.colorFor(name, this.queues.findIndex(q => q.name === name));
            $head.append($('<th/>').append(
                $('<span/>', {'class': 'aaxis-queue-mon__legend-swatch', style: 'background:' + color}),
                $('<span/>', {text: name})
            ));
        });
    ...
    private onPreview(event: any): void {
        event.preventDefault();
        ...
        const $messages = this.$el.find('[data-role="messages"]').empty().append(
            $('<p/>', {'class': 'aaxis-queue-mon__empty', text: __('aaxis.tools.queue_monitor.loading')})
        );
        ...
        fetch(this.messagesUrl(name), {
            ...
            .then(({ok, data}) => {
                ...
                this.renderMessages($messages, (data.messages || []) as QueueMessage[]);
            })
            .catch(() => {
    ...
    private renderMessages($container: any, messages: QueueMessage[]): void {
        $container.empty();
    ...
    private prettyPayload(payload: string): string {
        try {
    ...
    private initSplitter(): void {
        ...
        this.$el.on('mousedown.aaxisQueueMon', '[data-role="v-splitter"]', (event: any) => {
            event.preventDefault();
