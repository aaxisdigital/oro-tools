...
class ApiCollectionComponent extends BaseComponent {
    ...
    bindEvents() {
        ...
        this.$el.on('input.aaxisApi', '[data-role="filter"]', (e) => {
        ...
        this.$el.on('click.aaxisApi', '[data-role="kv-del"]', (e) => {
        ...
        this.$el.on('input.aaxisApi change.aaxisApi', '[data-role="url"], [data-role="body-type"], '
            + '[data-role="params-rows"] input, [data-role="headers-rows"] input', () => this.markDirty());
    ...
    openMenu(anchor, id, items) {
        ...
        this.$menu = $menu;
        setTimeout(() => {
            $(document).on('mousedown.aaxisApiMenu', (e) => {
    ...
    kvRow(key, value) {
        return $('<div/>', { 'class': 'aaxis-api__kv-row' }).append($('<input/>', { type: 'text', 'class': 'form-control aaxis-api__kv-key', placeholder: __('aaxis.tools.api_collection.key'), value: key, spellcheck: false }), $('<input/>', { type: 'text', 'class': 'form-control aaxis-api__kv-value', placeholder: __('aaxis.tools.api_collection.value'), value: value, spellcheck: false }), $('<button/>', { type: 'button', 'class': 'aaxis-api__kv-del', 'data-role': 'kv-del', title: __('aaxis.tools.api_collection.menu_remove'), html: '&times;' }));
    }
    readKv(which) {
        const result = [];
        this.$el.find('[data-role="' + which + '-rows"] .aaxis-api__kv-row').each((_, el) => {
            const $row = $(el);
    ...
    onTab(event) {
        ...
        this.$el.find('[data-role="panel"]').each((_, el) => $(el).prop('hidden', String($(el).data('tab')) !== tab));
    ...
    onSend() {
        ...
        this.$el.find('[data-role="result"]').html('').append($('<div/>', { 'class': 'aaxis-api__result-empty', text: __('aaxis.tools.api_collection.sending') }));
        const node = this.findNode(this.activeId);
    ...
    renderResult() {
        ...
        if (this.resultTab === 'headers') {
            ...
            res.headers.forEach(h => $h.append($('<div/>').append($('<span/>', { 'class': 'aaxis-api__res-hkey', text: h.key + ': ' }), $('<span/>', { text: h.value }))));
            $panel.append($h);
        }
        else {
            ...
            if (mode) {
                try {
                    CodeMirror.runMode(text, mode.mode, pre);
                }
                catch (e) {
    ...
    renderRuns() {
        ...
        this.runs.forEach(run => {
            ...
            $li.append($('<div/>', { 'class': 'aaxis-api__run-line' }).append($('<span/>', { 'class': 'aaxis-api__run-method ' + (METHOD_CLASS[run.method] || ''), text: run.method }), $('<span/>', { 'class': 'aaxis-api__run-status ' + statusClass, text: run.status !== null ? String(run.status) : '—' }), $('<span/>', { 'class': 'aaxis-api__run-name', text: run.name || '', title: run.name || '' })));
            $li.append($('<div/>', { 'class': 'aaxis-api__run-url', text: run.url, title: run.url }));
            $li.append($('<div/>', { 'class': 'aaxis-api__run-foot' }).append($('<span/>', { text: run.runAt ? new Date(run.runAt).toLocaleString() : '' }), $('<span/>', { text: this.formatSize(run.sizeBytes || 0) })));
            $runs.append($li);
        });
    }
    // --- cURL import / export ------------------------------------------------
    ...
    parseCurl(raw) {
        ...
        const unq = (t) => (/^['"].*['"]$/.test(t) ? t.slice(1, -1) : t);
        let method = '';
        let url = '';
        const headers = [];
        let body = '';
